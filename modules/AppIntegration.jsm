...
let baseURL = "resource://abprime/modules/";
...
WindowWrapper.prototype =
{
  ...
  handleLinkClick: function (/**Event*/ event, /**String*/ linkTarget)
  {
    ...
    this.window.openDialog("chrome://abprime/content/subscriptionSelection.xul", "_blank",
  ...
  openReportDialog: function()
  {
    ...
    if (wnd)
      ...
      this.window.openDialog("chrome://abprime/content/sendReport.xul", "_blank", "chrome,centerscreen,resizable=no", this.window.content, this.getCurrentLocation());
  },
  ...
  toggleSidebar: function()
  {
    ...
    else
    {
      ...
      if (sidebar && (!Prefs.detachsidebar || !sidebar.hidden))
      {
        ...
        this.E("abp-sidebar-browser").setAttribute("src", sidebar.hidden ? "chrome://abprime/content/sidebar.xul" : "about:blank");
        sidebar.hidden = !sidebar.hidden;
        if (sidebar.hidden)
          this.getBrowser().contentWindow.focus();
      }
      else
        this.detachedSidebar = this.window.openDialog("chrome://abprime/content/sidebarDetached.xul", "_blank", "chrome,resizable,dependent,dialog=no");
    }
  },
  ...
  blockItem: function(/**Node*/ node, /**RequestEntry*/ item)
  {
    ...
    this.window.openDialog("chrome://abprime/content/composer.xul", "_blank", "chrome,centerscreen,resizable,dialog=no,dependent", [node], item);
  }
};
...
function addSubscription()
{
  ...
  function notifyUser()
  {
    ...
    {
      wrapper.addTab("chrome://abprime/content/firstRun.xul");
    }
    else
    {
      Utils.windowWatcher.openWindow(wrapper ? wrapper.window : null,
                                     "chrome://abprime/content/firstRun.xul",
  ...
  if (addSubscription)
  {
    ...
    request.open("GET", "chrome://abprime/content/subscriptions.xml");
